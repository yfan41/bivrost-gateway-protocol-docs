...
## 8.1. HTTP 通讯时，返回"无法访问此网站(ERR_CONNECTION_REFUSED)" {#connection-refused}

请检查当前使用的计算机能否访问网关管理页面。如果不能，请检查网关是否开机，当前计算机与网关的网络连接是否正常。

## 8.2. HTTP 通讯时，返回 `"errorCode":3` {#errorcode-3}
...
## 8.3. MODBUS 或 MQTT 通讯时，没有收到任何数据 {#no-data}
...
## 8.4. MODBUS 通讯时，在给定的地址位无法获取有效数据 {#modbus-invalid-address}
